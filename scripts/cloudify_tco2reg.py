...
def parse_args():
    ...
    parser.add_argument(
        "dataset_name", help="The name of the dataset displayed for users."
    )
    parser.add_argument(
        "mode", choices=["refs", "norefs"], help="Specify 'refs' or 'norefs'."
    )
    parser.add_argument(
        "paths",
        nargs="+",
        help="Paths to dataset files (single for 'refs', multiple for 'norefs').",
    )
...
if __name__ == "__main__":  # This avoids infinite subprocess creation
    ...
    if refs == "refs":
        ...
        fsmap = fsspec.get_mapper(
            source, remote_protocol="file", lazy=True, cache_size=0
        )
        ds = xr.open_dataset(fsmap, engine="zarr", chunks=chunks, consolidated=False)
    else:
    ...
    if l_lossy:
        ds = xr.apply_ufunc(
            lossy_compress, ds, dask="parallelized", keep_attrs="drop_conflicts"
        )
    dvs = []
    l_el = False
