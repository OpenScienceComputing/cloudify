...
#    NGC_PROD_CAT="https://data.nextgems-h2020.eu/catalog.yaml"
    ...
    dsadd=["ICON.ngc4008", "IFS.IFS_9-FESOM_5-production.2D_hourly_healpix512",
...
def reset_encoding_get_mapper(dsid,ds,desc=None):
    ...
    if sp:
        ...
        mapper_dict[sp]=fsspec.get_mapper(
                sp,**use_options
                )
...
def set_compression(ds):
    for var in ds.data_vars:
        ...
        ds[var].encoding = {
                'compressor': numcodecs.Blosc(cname='lz4', clevel=5, shuffle=1, blocksize=0),
                }
    return ds
...
def adapt_for_zarr_plugin_and_stac(dsid,ds):
    ...
    if not desc:
        ...
        ds.attrs["description"]="Simulation data from project '"+project+ "' produced by Earth System Model '"+ source+"' and run by institution '"+institute+"' for the experiment '"+exp+"'"
    if "time" in ds.variables:
        ...
        for att in ["units","calendar"]:
            if ( ds["time"].attrs.get(att,None) and not ds["time"].encoding.get(att,None)):
