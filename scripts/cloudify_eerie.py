...
def get_dsone(cat, model:str, realm:str, coords:list):
    for dsname in ["2d_monthly_mean","monthly_node","2D_monthly_avg"]:
        try:            
            dscoords=(
                cat[
                    f"model-output.{model}.{realm}.native.{dsname}"
                ](chunks=None)
                .to_dask()
                .reset_coords()[coords].chunk()
            ) 
            return dscoords
        except:
            pass
...
def add_eerie(mapper_dict: dict, dsdict: dict) -> (dict, dict):
    ...
    dsonedict = {
        "icon_atmos":get_dsone(cat, "icon-esm-er.hist-1950.v20240618", "atmos", ["lat", "lon", "cell_sea_land_mask"]),
        "icon_ocean":get_dsone(cat, "icon-esm-er.hist-1950.v20240618", "ocean", ["lat", "lon", "cell_sea_land_mask"]),
        "icon_land":get_dsone(cat, "icon-esm-er.hist-1950.v20240618", "atmos", ["lat", "lon", "cell_sea_land_mask"]),
        "ifs-fesom_atmos":get_dsone(cat, "ifs-fesom2-sr.hist-1950.v20240304", "atmos", ["lat", "lon"]),
        "ifs-fesom_ocean":get_dsone(cat, "ifs-fesom2-sr.eerie-spinup-1950.v20240304","ocean", ["lat", "lon", "coast"]),
        "ifs-amip-tco1279_atmos":get_dsone(cat, "ifs-fesom2-sr.hist-1950.v20240304", "atmos", ["lat", "lon"]),        
    }
