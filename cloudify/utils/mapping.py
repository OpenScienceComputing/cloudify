...
GRID_LABEL_MAPPING = {"native": "gn", "gr025": "gr", "gn": "gn", "gr": "gr"}
...
def set_realm(item_title: str, item_ds):
    c = next(
        (b for b in item_title.split(" ") if b in list(REALM_MAPPING.keys())), None
    )
    if c:
...
def set_grid_label(item_title: str, item_ds):
    c = next(
        (b for b in item_title.split(" ") if b in list(GRID_LABEL_MAPPING.keys())), None
    )
    if c:
...
def set_frequency(item_title: str, item_ds):
    ...
    c = next(
        (FREQUENCY_MAPPING.get(b) for b in item_splits if b in FREQUENCY_MAPPING), None
    )
    if c:
    ...
    inp = next(
        (a for a in item_splits if a.startswith("PT") or a.startswith("P1")), None
    )
    if inp:
...
def map_expname_to_experiment(
    local_conf: Dict[str, str], exp_name_lower: str
) -> Dict[str, str]:
    if "d3hp" in exp_name_lower:
        if "aug" in exp_name_lower:
            local_conf["experiment_id"] = (
                "d3hp003aug"
                if not local_conf.get("experiment_id")
                else local_conf["experiment_id"] + "-aug"
            )
        if "feb" in exp_name_lower:
            local_conf["experiment_id"] = (
                "d3hp003feb"
                if not local_conf.get("experiment_id")
                else local_conf["experiment_id"] + "-feb"
            )
    ...
    if "rcbmf" in exp_name_lower:
        local_conf["experiment_id"] = (
            "rcbmf"
            if not local_conf.get("experiment_id")
            else local_conf["experiment_id"] + "-rcbmf"
        )
    ...
    if "deepoff" in exp_name_lower:
        local_conf["experiment_id"] = (
            "deepoff"
            if not local_conf.get("experiment_id")
            else local_conf["experiment_id"] + "-deepoff"
        )
        if "ng5" not in exp_name_lower and "fesom" not in exp_name_lower:
            local_conf["experiment_id"] += "-amip"
...
def map_expname_to_source(
    ...
    if not local_conf.get("source_id"):
        ...
        if "icon" in exp_name_lower:
            ...
            if (
                "fesom" in exp_name_lower or "ng5" in exp_name_lower
            ):  # ng5 only in fesom?
            ...
            local_conf["source_id"] = (
                exp_name_lower.strip(".").strip("_").strip(":").strip("-")
            )
    return local_conf
