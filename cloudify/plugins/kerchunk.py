...
class KerchunkPass(Plugin):
    ...
    def dataset_router(self, deps: Dependencies):
        ...
        @router.get('/{key:path}')
        async def get_chunk(key: str, dataset: xr.Dataset = Depends(deps.dataset),use_cache=False):
            global gctrigger
            if "source" in dataset.encoding:
                ...
                raise HTTPException(status_code=404, detail=f"Dataset ist not kerchunk-passable")
            #if key in fsmap:
            if True:
                if ".zmetadata" == key:
