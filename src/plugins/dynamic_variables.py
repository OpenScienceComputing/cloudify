...
class DynamicAdd(Plugin):
    ...
    def dataset_router(self, deps: Dependencies):
        ...
        def evaluate_groupby(variable_name: str, groupby_coord: str, groupby_action: str, request: Request, dataset: xr.Dataset = Depends(get_dataset)):
            if variable_name not in dataset.variables:
                raise HTTPException(status_code=404, detail=f"Variable '{variable_name}' not found in dataset")

            coord_name = groupby_coord.split('.')[0]
            if coord_name not in dataset.dims and coord_name not in dataset.coords:
                raise HTTPException(status_code=404, detail=f"Groupby coordinate '{groupby_coord}' not found in dataset")

            if groupby_action not in ["mean", "min", "max", "count", "first", "last", "sum"]:
                raise HTTPException(status_code=404, detail=f"Groupby action '{groupby_action}' not recognized")
