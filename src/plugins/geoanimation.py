...
#                crs=ccrs.Mercator(),
    ...
    def change_units(self,ds):
        if "lat" in ds.coords and "lat" not in ds.dims and not ds["lat"].attrs["units"].startswith("degrees"):
...
#            else:
    ...
    def unstacke_and_nninterpol(self,ds):
        if "value" in ds.dims and "lat" not in ds.dims:
            ...
            status="1."
            try:
                unstack=ds.rename({'value':'latlon'}).set_index(latlon=("lat","lon")).unstack("latlon")
                status="unstack"
                ...
                print(target_lons)
                status="target_lons"
                interp=unstack.interpolate_na(dim="lon",method="linear",period=360.0)
                status="interp"
...
#        mima = [await client.compute(ds.min()),await client.compute(ds.max())]
        cmin,cmax=round(mima[0].data.item(),6)-10**-6,round(mima[1].data.item(),6)
        await client.close()
        return ds, (cmin,cmax)
...
#            plot = plot.result()
            ...
            plot = pn.Column(
                    ...
                    pn.pane.HTML(
                        ...
                        'The dataset is available via:<br></h1><h5> https://eerie.cloud.dkrz.de/datasets/'+xid+' </h5>'
                        )
                    )
    ...
    def dataset_router(self, deps: Dependencies):
        ...
        async def get_custom_plot(var_name: str, kind:str, cmap:str, request: Request, selection:str =  None, dataset: xr.Dataset = Depends(get_dataset)):
            ...
            if cmap not in hv.plotting.list_cmaps():
                raise HTTPException(status_code=404, detail=f"Variable '{cmap}' not found in colormaps")
            llati = False
            llongi = False
            if not "lat" in dataset.coords :
                if not "latitude" in dataset.coords :
                    raise HTTPException(status_code=404, detail=f"Need 'lat' or 'latitude' coordinate")
                else:
            ...
            if not "lon" in dataset.coords:
                if not "longitude" in dataset.coords :
                    raise HTTPException(status_code=404, detail=f"Need 'lon' or 'longitude' coordinate")
                else:
